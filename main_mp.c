// Parallel Programming Homework 1 / Parallel Graph Triangle Counting with OpenMP
...
/* HERE STARTS MY PROGRAM NOT THE PREPROCESSING*/
    ...
    {
         #pragma omp  for collapse(1) schedule(dynamic, 8) nowait
          for (uint32_t k=0; k < (M-1); k++) {
            ...
            for (uint32_t p=RowPtr[k]; p<RowPtr[k+1];p++)
            {
                ...
                nVal[p] = sum_local;
                C[k] = C[k] + nVal[p]; // Triangles for each Row (graph)
            }
...
//          printf("\nThe number of triangles incident of vertex %d, is %d.",k,C[k]); //print the c = C * e/2
     ...
     printf("\n%d\n",all); // Print the sum of all triangles.
